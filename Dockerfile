...
RUN dpkg --add-architecture i386 && \
    apt-get -y update && \
    apt-get -y install -yq libstdc++6:i386 zlib1g:i386 libncurses5:i386 --no-install-recommends && \
...
# Install Gradle
RUN wget https://services.gradle.org/distributions/gradle-3.2-bin.zip
RUN unzip gradle-3.2-bin.zip
RUN mv gradle-3.2 /usr/local/
RUN rm gradle-3.2-bin.zip
...
ENV GRADLE_HOME /usr/local/gradle-3.2
